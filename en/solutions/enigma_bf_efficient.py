...
def job_callback(job): # executed at the client
    ...
    if (job.status == dispy.DispyJob.Finished  # most usual case
        ...
        if job.id: # job may have finished before 'main' assigned id
            ...
            rotor_choice, ring_choice, start_pos = job.result # returns results from job
            if (start_pos != "null"):
                found = True
                dispy.logger.info( 'Machine setting found: job "%i" returned "%s" with ring "%s" using "%s", %s jobs pending', job.id, rotor_choice, ring_choice, start_pos, len(pending_jobs) )
...
if __name__ == '__main__':
    ...
    pending_jobs = {}
    cluster = dispy.JobCluster(find_rotor_start, nodes=server_nodes, callback=job_callback, loglevel=logging.INFO)
        ...
        while (r1 < len(ring)) and (found == False):
            while (r2 < len(ring)) and (found == False):
                ...
                while (rot < len(rotor)) and (found == False):
                    ...
                    print( 'Trying rotor "%s" with ring "%s" ...' % (rotor_choice, ring_choice) )
